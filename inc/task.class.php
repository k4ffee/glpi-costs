...
class PluginCostsTask extends CommonDBTM
{
                ...
                if ($task->fields['state'] == Planning::DONE) {
                    ...
                    if ($cost_time > 0) {
                        if (!$task->fields['is_private'] || $cost_config->fields['cost_private']) {
                            ...
                            if ($config->fields['taskdescription_as_costs_name']) {
                                $name = self::taskContentAsCostsName($task->fields['content']);
                            }
    ...
    public static function preTaskUpdate(TicketTask $task): void
    {
        ...
        if (PluginCostsTicket::isBillable($task->fields['tickets_id'])) {
            ...
            if ($task->input['state'] == Planning::DONE) {
                ...
                $name = self::taskContentAsCostsName($content);
    ...
    {
        $content = Sanitizer::sanitize(RichText::getTextFromHtml($content));
        // longtext to text
        $tagline = " \n" . __('Automatically generated by GLPI') . ' -> Costs Plugin';
    ...
    /**
     * Format task content as name cleaning html tags and reduce it to varchar length 255
     *
     * @param string $content
     * @return string
     */
    private static function taskContentAsCostsName(string $content): string
    {
        $content = Sanitizer::sanitize(RichText::getTextFromHtml($content));
